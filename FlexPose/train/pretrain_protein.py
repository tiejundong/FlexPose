...
from model.layers import ProteinPretrain
from model.loss import PocketLossFunction
from model.param_setting import get_pocket_params
...
def train(rank, world_size, port, args):
    ...
    my_model = ProteinPretrain(args).to(rank)
    if rank == 0 or not args.use_multi_gpu:
    ...
    train_dataset = PocketDataset('train', args, train_list, cache_path=args.cache_path)
    val_dataset = PocketDataset('val', args, val_list, cache_path=args.cache_path)
    ...
    loss_object = PocketLossFunction(args).to(rank)
    ...
    dic_opt = get_pocket_params(my_model, loss_object, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str,
                        default='/root/autodl-tmp/drug/data/prepare_pocket_npz/',
    ...
    parser.add_argument('--n_batch', type=int, default=10000, help='number of batch')
    ...
    # feat mask
    parser.add_argument('--total_mask_rate', type=float, default=0.15, help='masking rate')
    parser.add_argument('--mask_rate_AAtype', type=float, default=0.30, help='masking rate')
    parser.add_argument('--mask_rate_MCcoor', type=float, default=0.10, help='masking rate')
    parser.add_argument('--mask_rate_SCcoor', type=float, default=0.30, help='masking rate')
    parser.add_argument('--mask_rate_CAnoise', type=float, default=0.30, help='masking rate')

    parser.add_argument('--CAnoise_dist', type=float, default=3, help='(A) CAnoise param')

    # loss weight
    parser.add_argument('--gamma_AAtype', type=float, default=1, help='gamma AAtype')
    parser.add_argument('--gamma_CAnoise', type=float, default=1, help='gamma CAnoise')
    parser.add_argument('--gamma_MCcoor', type=float, default=1, help='gamma MCcoor')
    parser.add_argument('--gamma_SCcoor', type=float, default=10, help='gamma SCcoor')
